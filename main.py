...
SUDO_USERS = [7417246937,8080218275]
    ...
    try:
        ...
        if action == "add":
            ...
            if target_user_id == OWNER_ID:
                await message.reply_text("**🚫 The owner cannot be removed from the sudo list contact @courses_hub2_bot .**")
...
keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🇮🇳ʙᴏᴛ ᴍᴀᴅᴇ ʙʏ🇮🇳" ,url=f"https://t.me/courses_hub2_bot") ],
                    [
                    InlineKeyboardButton("🔔ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ🔔" ,url="https://t.me/ARGETALLCOURSE") ],
                    [
                    InlineKeyboardButton("🦋ғᴏʟʟᴏᴡ ᴜs🦋" ,url="https://t.me/courses_hub2_bot")                              
                ],           
            ]
      )
...
async def edit_txt(client, message: Message):
    ...
    try:
        await message.reply_document(
            ...
            caption="📥**𝗘𝗱𝗶𝘁𝗲𝗱 𝗕𝘆 ➤ CR CHOUDHARY**"
        )
    except Exception as e:
...
async def upload(bot: Client, m: Message):
    if not is_authorized(m.chat.id):
        await m.reply_text("**🚫You are not authorized to use this bot contact .... @courses_hub2_bot .**")
        return
    ...
    await m.reply_text(f"<pre><code>『🪩 DONE ✅』</code></pre>")                 
